...
public abstract class AReactor extends SlimefunItem {
	...
	private void constructMenu(BlockMenuPreset preset) {
		...
		preset.addItem(22, new CustomItem(new MaterialData(Material.STAINED_GLASS_PANE, (byte) 15), " "), new MenuClickHandler() {
		...
		if(needsCooling()) {
			...
			for(int i : border_4) {
				preset.addItem(i, new CustomItem(new ItemStack(Material.BARRIER), "&cNo Coolant Required"), new MenuClickHandler() {
		...
		addItemHandler(new EnergyTicker() {
			...
			public double generateEnergy(final Location l, SlimefunItem sf, Config data) {
				...
				if(isProcessing(l)) {
					...
					if(port != null) {
						refill: for(int slot : getFuelSlots()) {
	...
	private static Inventory inject(Location l, int[] slots) {
		int size = BlockStorage.getInventory(l).toInventory().getSize();
		Inventory inv = Bukkit.createInventory(null, size);
		for(int i = 0; i < size; i++) {
			inv.setItem(i, new CustomItem(Material.COMMAND, " &4ALL YOUR PLACEHOLDERS ARE BELONG TO US", 0));
